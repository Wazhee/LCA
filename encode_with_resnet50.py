...
class RSNADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Ensure 3 channels for ResNet50 pretrained weights
...
def extract_image_embeddings(model, dataloader):
    ...
    with torch.no_grad():
        for imgs, patient_ids in tqdm(dataloader):
            ...
            vision_feat = model(imgs).detach().float().cpu().numpy()
...
rsna_train = RSNADataset(df_train, img_dir, transforms=RSNA_Transforms)
rsna_test   = RSNADataset(df_test, img_dir, transforms=RSNA_Transforms)
...
checkpoint = torch.load("checkpoints/resnet50_rsna_model.pth", map_location="cpu")
...
# ----------------------------
save_dir = "../CXR/RSNA_StyleEmbeddings/resnet_rsna_embeddings/"
